...
const GOV_BLUE = '#0d3b7a';
const GOV_BLUE_DARK = '#081f47';
const GOV_BLUE_LIGHT = '#eef3fb';
const GOV_TEXT = '#212529';
const GOV_GRAY = '#5b6770';
const GOV_GREEN = '#5a8f3c';
const GOV_GREEN_DARK = '#3f6b2a';
const GOV_BORDER = '#dde3ec';
...
export default function EKXARegistry() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f6f8', fontFamily: "'Open Sans', Arial, sans-serif" }}>
      ...
      <div style={{ background: '#06143a', color: '#9fb4d6', fontSize: 12, padding: '7px 0' }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <svg width="16" height="16" viewBox="0 0 100 100" style={{ flexShrink: 0 }}>
            <g stroke="#9fb4d6" strokeWidth="5" fill="none" strokeLinecap="round">
              <path d="M50 78 C 38 70, 32 58, 34 46 C 36 50, 40 52, 44 50" />
              <path d="M50 78 C 38 70, 32 58, 34 46 C 36 50, 40 52, 44 50" transform="scale(-1,1) translate(-100,0)" />
            </g>
          </svg>
          <span style={{ fontWeight: 600 }}>ΕΛΛΗΝΙΚΗ ΔΗΜΟΚΡΑΤΙΑ</span>
          <span style={{ opacity: 0.7 }}>Υπουργείο Ψηφιακής Διακυβέρνησης</span>
          <span style={{ marginLeft: 'auto', opacity: 0.75 }}>{todayStr}</span>
        </div>
      </div>

      <header style={{ background: GOV_BLUE }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 38, height: 38, borderRadius: 9, background: GOV_GREEN, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Building2 size={20} color="#fff" strokeWidth={2.2} />
            </div>
            <div style={{ color: '#fff', fontFamily: "'Roboto Slab', serif", fontWeight: 700, fontSize: 22, letterSpacing: -0.2, lineHeight: 1.1 }}>
              Κτηματολόγιο
              <span style={{ display: 'block', color: '#aebfe0', fontSize: 11.5, letterSpacing: 0.6, fontWeight: 500, fontFamily: "'Open Sans', sans-serif", marginTop: 2 }}>ΕΚΧΑ — NATIONAL CADASTRE AGENCY</span>
            </div>
          </div>
          ...
          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="ekxa-burger" style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 12, padding: 9, cursor: 'pointer' }}>
      ...
      <div style={{ background: '#fff', borderBottom: `1px solid ${GOV_BORDER}` }}>
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '10px 20px', fontSize: 12.5, color: GOV_GRAY, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
      ...
      {showAuthModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,30,60,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, maxWidth: 400, width: '100%', boxShadow: '0 20px 50px rgba(0,0,0,0.35)', overflow: 'hidden' }}>
              ...
              <form onSubmit={handleLogin}>
                <div style={{ marginBottom: 12 }}>
                  <input type="email" autoFocus required value={loginEmail} onChange={(e) => { setLoginEmail(e.target.value); setAuthError(''); }} placeholder="Email"
                    style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: `1.5px solid ${GOV_BORDER}`, fontSize: 15, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <input type="password" required value={loginPassword} onChange={(e) => { setLoginPassword(e.target.value); setAuthError(''); }} placeholder="Κωδικός / Password"
                    style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: authError ? '1.5px solid #b23b3b' : `1.5px solid ${GOV_BORDER}`, fontSize: 15, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                {authError && <div style={{ fontSize: 12.5, color: '#b23b3b', marginBottom: 10 }}>{authError}</div>}
                <button type="submit" disabled={authSubmitting} style={{ width: '100%', background: authSubmitting ? '#9aa7bd' : GOV_BLUE, color: '#fff', border: 'none', padding: '12px', borderRadius: 12, fontSize: 14.5, fontWeight: 700, cursor: authSubmitting ? 'default' : 'pointer', fontFamily: 'inherit', marginTop: 4 }}>
      ...
      {toast && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 50, background: toast.type === 'error' ? '#7a2424' : '#1e5631', color: '#fff', padding: '14px 18px', borderRadius: 12, maxWidth: 380, fontSize: 14, lineHeight: 1.45, boxShadow: '0 8px 24px rgba(0,0,0,0.3)', display: 'flex', gap: 10, alignItems: 'flex-start', borderLeft: '4px solid ' + (toast.type === 'error' ? '#c9595b' : '#7fc494') }}>
      ...
      <main style={{ maxWidth: 1180, margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        {view === 'register' && !isOfficer && (
          <div style={{ textAlign: 'center', padding: '60px 24px', background: '#fff', border: `1px solid ${GOV_BORDER}`, borderRadius: 14 }}>
...
function pillBtnStyle() { return { display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 24, border: 'none', background: GOV_GREEN, color: '#fff', fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }; }

function NavBtn({ active, onClick, label, sub, full }) {
  return (
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', alignItems: full ? 'flex-start' : 'center', gap: 1, padding: '8px 16px', borderRadius: 12, border: 'none', cursor: 'pointer', background: active ? '#fff' : 'rgba(255,255,255,0.12)', color: active ? GOV_BLUE : '#fff', fontFamily: 'inherit', width: full ? '100%' : 'auto', textAlign: full ? 'left' : 'center' }}>
...
function StatusPill({ status }) {
  ...
  return (
    <span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-start', background: s.bg, color: s.text, borderRadius: 8, padding: '5px 11px', borderLeft: `3px solid ${s.text}` }}>
...
function SearchView({ query, setQuery, filtered, loading, onOpen, effectiveStatus, onGoRegister, isOfficer, totalCount }) {
  return (
    <div>
      <div style={{ background: '#fff', border: `1px solid ${GOV_BORDER}`, borderTop: `4px solid ${GOV_BLUE}`, borderRadius: 14, padding: '26px 28px', marginBottom: 24 }}>
      ...
      <div style={{ position: 'relative', marginBottom: 22 }}>
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="π.χ. Πλάκα, Αθήνα — ή AT-204819"
          style={{ width: '100%', padding: '13px 14px 13px 42px', borderRadius: 12, border: `1.5px solid ${GOV_BORDER}`, fontSize: 15, fontFamily: 'inherit', outline: 'none', background: '#fff', boxSizing: 'border-box' }} />
      </div>

      {loading ? (
        <div style={{ color: '#8a95a3', fontSize: 14, padding: '40px 0', textAlign: 'center' }}>Φόρτωση μητρώου…</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '52px 24px', background: '#fff', border: `1px solid ${GOV_BORDER}`, borderRadius: 14 }}>
          ...
          {isOfficer && !query && <button onClick={onGoRegister} style={{ background: GOV_GREEN, color: '#fff', border: 'none', padding: '11px 24px', borderRadius: 24, fontSize: 13.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Καταχώριση Ακινήτου</button>}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 1, border: `1px solid ${GOV_BORDER}`, background: GOV_BORDER, borderRadius: 14, overflow: 'hidden' }}>
          {filtered.map((r) => (
            <button key={r.parcel_id} onClick={() => onOpen(r)} style={{ display: 'flex', alignItems: 'center', gap: 14, textAlign: 'left', background: '#fff', border: 'none', padding: '15px 18px', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}>
              <div style={{ width: 34, height: 34, borderRadius: 12, background: GOV_BLUE_LIGHT, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        {isOfficer && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onEdit} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fff', border: `1.5px solid ${GOV_BLUE}`, color: GOV_BLUE, padding: '7px 13px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}><Pencil size={13} /> Επεξεργασία</button>
            {!confirmingDelete ? (
              <button onClick={() => setConfirmingDelete(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fff', border: '1.5px solid #b23b3b', color: '#b23b3b', padding: '7px 13px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}><Trash2 size={13} /> Αφαίρεση</button>
            ) : (
              <button onClick={onDelete} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#b23b3b', border: '1.5px solid #b23b3b', color: '#fff', padding: '7px 13px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}><Trash2 size={13} /> Επιβεβαίωση</button>
            )}
          </div>
        )}
      </div>

      <div style={{ background: '#fff', border: `1px solid ${GOV_BORDER}`, borderRadius: 14, overflow: 'hidden' }}>
...
function RegisterView({ form, setForm, onSubmit, submitting, editingRecord }) {
  ...
  const inputStyle = { width: '100%', padding: '11px 13px', borderRadius: 12, border: `1.5px solid ${GOV_BORDER}`, fontSize: 14.5, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', background: '#fff' };
  const labelStyle = { fontSize: 12.5, fontWeight: 700, color: GOV_BLUE_DARK, marginBottom: 6, display: 'block' };

  return (
    <div>
      <div style={{ background: '#fff', border: `1px solid ${GOV_BORDER}`, borderTop: `4px solid ${GOV_BLUE}`, borderRadius: '14px 14px 0 0', padding: '26px 28px', marginBottom: 4 }}>
      ...
      <form onSubmit={onSubmit} style={{ background: '#fff', border: `1px solid ${GOV_BORDER}`, borderTop: 'none', borderRadius: '0 0 14px 14px', padding: '24px 28px 8px' }}>
        ...
        <div style={{ paddingBottom: 24 }}>
          <button type="submit" disabled={submitting} style={{ width: '100%', background: submitting ? '#9aa7bd' : GOV_GREEN, color: '#fff', border: 'none', padding: '14px', borderRadius: 24, fontSize: 15, fontWeight: 700, cursor: submitting ? 'default' : 'pointer', fontFamily: 'inherit' }}>
